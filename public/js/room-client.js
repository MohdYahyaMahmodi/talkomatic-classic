  ...
  Object.keys(messages).forEach((userId) => {
    ...
    if (chatDiv) {
      ...
      if (userId === currentUserId) {
        ...
        updateOtherUserMessage(chatDiv, displayMessage);
      }
    }
  });
}
...
function updateOtherUserMessage(element, message) {
  ...
  element.dataset.rawMessage = message;
  const display = censorEnabled ? applyWordCensor(message) : message;
...
function displayChatMessage(data) {
  ...
  if (data.userId === currentUserId) {
    ...
    updateOtherUserMessage(chatDiv, displayText);
  }
}
...
window.addEventListener("load", () => {
  ...
  socket.emit("censor setting", { enabled: censorEnabled });
